...
public class WindowSearchProvider : ScriptableObject, ISearchWindowProvider
{
    ...
    public List<SearchTreeEntry> CreateSearchTree(SearchWindowContext context)
    {
        ...
        Assembly[] assemblies = AppDomain.CurrentDomain.GetAssemblies(); //how it gets the idea about what is an assembly?

        foreach (var assembly in assemblies) //that can cause the problem
        {
        ...
        elements.Sort((entry1, entry2) =>
        {
            ...
            for (int i = 0; i < splits1.Length; i++)
            {
                ...
                if (value != 0)
                {
                    //Make sure the leaves go before nodes
        ...
        foreach (SearchContextElement element in elements)
        {
            ...
            //To create groups in the window
            ...
            //For the actual type we are creating just an entry, not an entry group
    ...
    public bool OnSelectEntry(SearchTreeEntry SearchTreeEntry, SearchWindowContext context)
    {
        ...
        //var newPosition = new Rect(graphMousePosition, new Vector2());
        graphView.CreateNode(node.GetType(), graphMousePosition);
        // node.SetPosition(new Rect(graphMousePosition, new Vector2()));
        // graphView.Tree.CreateNode()
